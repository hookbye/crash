...
void GameLayer::fallDown()
{
	...
	{
		icon = (GameIcon*)stables->objectAtIndex(i);
		int tx = icon->getX();
		int ty = icon->getY();
		for(int j=0;j<row;j++)
		{
	...
	for(int i=0;i<icons->count();i++)
	{
		...
		if(fallNum>0)
		{
			for(int k=ty;k>ty-fallNum;k--)
			{
				...
				/*if(tmpIcon && tmpIcon->isStable())
					fallNum++;*/
			}
			/*while(getIconByLoc(tx,ty-fallNum) && getIconByLoc(tx,ty-fallNum)->isStable())
			{
...
void GameLayer::changeCell(MoveDirect direct)
{
	...
	//CCLog("%d,%d ΩªªªŒª÷√ «%d,%d",x,y,sx,sy);
